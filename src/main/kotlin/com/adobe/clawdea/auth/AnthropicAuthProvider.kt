...
class AnthropicAuthProvider(
    ...
    private val apiBaseUrl: String = "https://api.anthropic.com",
) : AuthProvider {

    override val id = "anthropic"

    constructor() : this(
        apiKey = { ClawDEASettings.getInstance().getApiKey() },
    ...
    override fun validate(): AuthValidation {
        return if (isConfigured()) {
            ...
            AuthValidation(
                valid = false,
                message = "No API key configured. Set it in Settings > Tools > ClawDEA, or export ANTHROPIC_API_KEY in your shell.",
            )
        }
    }
    ...
    override fun testConnection(): ConnectionTestResult {
        ...
        val body = """{"model":"claude-sonnet-4-20250514","max_tokens":1,"messages":[{"role":"user","content":"hi"}]}"""
        val request = HttpRequest.newBuilder()
            .uri(URI.create("$apiBaseUrl/v1/messages"))
