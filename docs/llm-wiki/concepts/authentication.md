...
- **All auth subprocesses run with a login-shell-sourced environment.** `defaultAuthProcessEnvironment()` ([SubscriptionAuth.kt](../../../src/main/kotlin/com/adobe/clawdea/auth/SubscriptionAuth.kt)) calls `CliEnvironment.applyTo()` to load NVM, Homebrew, and other shell-init entries, then back-fills missing keys from `System.getenv()`. Both `SubscriptionAuth` (via `DefaultProcessRunner`) and `SubscriptionAuthProbe` receive this through an injectable `environmentProvider` — so tests supply a controlled map without a real shell, and Finder/Dock launches find `claude` without extra PATH config.
