...
- [BridgeExpandingHandler.kt](../../../src/main/kotlin/com/adobe/clawdea/commands/handlers/BridgeExpandingHandler.kt) — expands an in-plugin prompt template, then forwards to the CLI (used by `/learn`, `/seed-wiki`, `/seed-workspace`)
...
### Surfacing skills to Claude
Skill discovery (above) only populates the registry and the `/skills` picker. To make Claude itself aware of the available skills, `CliProcess` injects a static catalog into the CLI system prompt at session start:
...
- The system-prompt skill catalog (`buildSkillCatalogPrompt`) is built once when `CliProcess` starts and is **not** updated mid-session. Skills discovered or shadowed after the CLI launches won't appear to Claude until the process restarts, even though they show up in the `/skills` picker immediately.
- `/refresh-wiki` is a `LocalHandler` (still tagged `CommandCategory.BRIDGE` for UI grouping) whose handler calls `DriftDetectionService.rescan()` directly to collect events, then `DriftDetectionService.runAuthorNow(events)` to author fixes in a dedicated out-of-band `claude -p` subprocess — not the interactive chat CLI turn. The post-commit `CommitWikiDriftDetector` is what triggers the automatic in-plugin rescan (via `rescan()`'s own auto-apply step); `/refresh-wiki` is the manual counterpart that lets the user review/apply the same events on demand.
