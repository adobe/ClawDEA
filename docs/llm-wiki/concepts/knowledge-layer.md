...
- When an assembly pass yields a blank payload (all sources empty or failed, since `PrimerAssembler` skips blank bodies), `nextCachedPayload` keeps the previously cached primer rather than evicting it. This prevents transient FS/VFS/git errors from silently erasing a working primer for the next CLI start ([PrimerService.kt](../../../src/main/kotlin/com/adobe/clawdea/knowledge/primer/PrimerService.kt)).
- Concept pages live under `<wikiDir>/concepts/<slug>.md`, three levels below the wiki root. Source-file links must therefore be three-dot-up relative paths pointing into `src/` — anything else breaks click-through and drift detection ([CodeRenameDetector.kt](../../../src/main/kotlin/com/adobe/clawdea/knowledge/drift/CodeRenameDetector.kt)).
