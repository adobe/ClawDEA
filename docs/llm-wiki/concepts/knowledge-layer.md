...
- The wiki path is resolved by **`WikiLocator`** (project service) — a single source of truth. In default mode (no `<projectBase>/.clawdea/config.json`) it returns `<projectBase>/<claudeDirName>/<wikiSubdir>` (defaults `.claude/wiki/`, configurable in `ClawDEASettings`). In team mode (config file present with non-blank `wikiPath`) it returns `<projectBase>/<wikiPath>`. All wiki-aware code (primer sources, drift detection, MCP wiki tools, chat panel, `/promote-to-wiki`) routes through `WikiLocator.getInstance(project).wikiDir()` — never construct the wiki path manually ([WikiLocator.kt](../../../src/main/kotlin/com/adobe/clawdea/knowledge/wiki/WikiLocator.kt)).
