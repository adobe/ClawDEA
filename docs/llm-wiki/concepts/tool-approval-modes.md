...
- Mode → CLI argv mapping (`buildPermissionArgs` in [CliProcess.kt](../../../src/main/kotlin/com/adobe/clawdea/cli/CliProcess.kt)), which now takes two parameters: the approval mode and a chat mode (see [ChatModeUi](../../../src/main/kotlin/com/adobe/clawdea/settings/ChatModeUi.kt) for the chat mode mapping). The chat mode takes precedence when it maps to a value:
  ...
  - Chat mode `plan` → `--permission-mode plan` (overrides `allow-safe`'s `auto`); chat mode `auto` / `ask` → no chat-mode flag, fall through to the approval mode.
