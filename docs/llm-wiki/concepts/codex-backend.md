...
- **Codex sign-in has no paste-code step.** Unlike Claude's `SubscriptionAuth`, `CodexSubscriptionAuth.signIn()` just streams `codex login` and lets codex open the browser / device-auth itself. Status comes from `codex login status` plain text (no `--json`, no email/tier), parsed **negative-case-first** because "not logged in" contains "logged in". Like `SubscriptionAuthProbe`, `CodexSubscriptionAuthProbe` sources the subprocess environment via `defaultAuthProcessEnvironment()` (injectable for tests), so it can find the `codex` binary even when IntelliJ was launched from Finder/Dock with a stripped PATH ([CodexSubscriptionAuth.kt](../../../src/main/kotlin/com/adobe/clawdea/auth/CodexSubscriptionAuth.kt), [CodexSubscriptionAuthProbe.kt](../../../src/main/kotlin/com/adobe/clawdea/auth/CodexSubscriptionAuthProbe.kt)).
