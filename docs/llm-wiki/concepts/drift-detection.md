...
- `CommitWikiDriftDetector` uses `lastSyncedCommit..HEAD` (resolved via `git merge-base --is-ancestor` to handle rebased-away SHAs) as the commit range. An empty or unreachable `lastSyncedCommit` is a first-run/no-baseline case that returns no events; the caller adopts current HEAD as the baseline. `DriftDetectionService.bumpSyncedCommit` advances `lastSyncedCommit` to HEAD after every successful rescan ([CommitWikiDriftDetector.kt](../../../src/main/kotlin/com/adobe/clawdea/knowledge/drift/CommitWikiDriftDetector.kt), [DriftDetectionService.kt](../../../src/main/kotlin/com/adobe/clawdea/knowledge/drift/DriftDetectionService.kt)).
...
- [DriftDetectionService.kt](../../../src/main/kotlin/com/adobe/clawdea/knowledge/drift/DriftDetectionService.kt) — rescan orchestrator, listener fan-out, `bumpSyncedCommit`
