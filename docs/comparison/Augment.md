...
| **Authored project knowledge** | `.claude/wiki/` (concepts/sources/index) read+searched via MCP. `/seed-wiki` bootstraps from project state. `learn-on-probe-miss` writes new pages on retrieval gaps. | None equivalent. | ClawDEA |
